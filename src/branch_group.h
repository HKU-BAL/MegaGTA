...
class BranchGroup {
  public:
    ...
    BranchGroup(SuccinctDBG *sdbg, int64_t begin_node, int max_branches = 4, int max_length = 0):
        sdbg_(sdbg), begin_node_(begin_node), max_branches_(max_branches), max_length_(max_length) {
    ...
    bool Search();
    bool Pop(AtomicBitVector &marked);
    size_t length() {
        if (branches_.size() == 0) {
            return 0;
        }
